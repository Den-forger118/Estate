...
// POST /api/v1/accounts/[userId]/reactivate — ADMIN only
export async function POST(_req: NextRequest, { params }: Params) {
  ...
  if (role !== "ADMIN") {
    return NextResponse.json({ error: "Forbidden — only ADMIN can reactivate accounts" }, { status: 403 })
  }
  ...
  return NextResponse.json({ ok: true, message: `Account ${user.email} has been reactivated.` })
}
